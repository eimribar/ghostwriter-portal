...
-- Step 9: Insert demo data for Jonathan (CEO)
DO $$
DECLARE
...
BEGIN
    -- Create Jonathan's user account (if not exists)
    INSERT INTO auth.users (id, email) 
    VALUES (gen_random_uuid(), 'jonathan@company.com')
    ON CONFLICT (email) DO NOTHING
    RETURNING id INTO jonathan_user_id;
    
    -- If user already existed, get the ID
    IF jonathan_user_id IS NULL THEN
        SELECT id INTO jonathan_user_id FROM auth.users WHERE email = 'jonathan@company.com';
    END IF;
    ...
    -- Create Jonathan's client record
    INSERT INTO clients (id, name, company, email, user_id, industry, status, portal_access, mobile_pin, content_preferences, brand_guidelines)
    VALUES (
        gen_random_uuid(),
        'Jonathan',
        'Tech Company Inc',
    ...
    -- Get the client ID if it already existed
    IF jonathan_client_id IS NULL THEN
        SELECT id INTO jonathan_client_id FROM clients WHERE email = 'jonathan@company.com';
    END IF;
    
    -- Create base prompts for CEO content (if they don't exist)
    INSERT INTO prompt_templates (id, name, category, system_message, is_active, is_default, settings)
    VALUES (
        gen_random_uuid(),
    ...
    RAISE NOTICE 'Jonathan CEO profile and prompts created successfully';
