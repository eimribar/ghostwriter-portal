...
-- Track client actions and notify admin
...
CREATE INDEX IF NOT EXISTS idx_activity_log_content_id ON client_activity_log(content_id);
...
CREATE POLICY "Allow all operations for authenticated" ON notifications
FOR ALL TO authenticated
USING (true)
WITH CHECK (true);

-- Step 6: Create RLS policies for activity log
CREATE POLICY "Allow all operations for anon" ON client_activity_log
FOR ALL TO anon
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all operations for authenticated" ON client_activity_log
FOR ALL TO authenticated
USING (true)
WITH CHECK (true);

-- Step 7: Create function to automatically create notifications on status change
  ...
  IF NEW.status IN ('client_approved', 'client_rejected', 'client_edited') AND 
    ...
    CASE NEW.status
      WHEN 'client_approved' THEN
        ...
        notification_title := client_name || ' approved content';
        notification_message := 'Content variant #' || NEW.variant_number || ' has been approved by ' || client_name;
      WHEN 'client_rejected' THEN
        notification_type := 'rejected';
        notification_title := client_name || ' rejected content';
      ...
      WHEN 'client_edited' THEN
        ...
        notification_title := client_name || ' edited content';
...
-- Step 8: Create trigger for automatic notifications
...
WHERE id = (SELECT id FROM generated_content WHERE status = 'admin_approved' LIMIT 1);
*/
...
UNION ALL
SELECT 
  'client_activity_log' as table_name,
  COUNT(*) as row_count
