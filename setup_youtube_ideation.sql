-- =====================================================
-- YOUTUBE IDEATION SYSTEM SETUP
-- Adds YouTube prompt template and content_ideas enhancements
-- =====================================================

-- Step 1: Add APIFY_API_KEY environment variable reminder
DO $$
BEGIN
  RAISE NOTICE '🔧 ENVIRONMENT SETUP REQUIRED:';
  RAISE NOTICE '   Add APIFY_API_KEY to Vercel environment variables';
  RAISE NOTICE '   Get from Apify dashboard: https://console.apify.com/account/integrations';
  RAISE NOTICE '';
END $$;

-- Step 2: Ensure content_ideas table supports YouTube source
-- Check if source_metadata column exists, if not add it
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE content_ideas ADD COLUMN source_metadata JSONB;
    RAISE NOTICE '✅ Added source_metadata column to content_ideas table';
  ELSE
    RAISE NOTICE '✅ source_metadata column already exists in content_ideas table';
  END IF;
END $$;

-- Step 3: Create the YouTube transcript prompt template
...
-- Step 4: Create a simpler fallback YouTube prompt
INSERT INTO prompt_templates (
  name,
  description,
  prompt_text,
...
-- Step 6: Update RLS policies to ensure YouTube ideas can be created
-- (Assuming content_ideas already has proper RLS policies)

-- Step 7: Create a function to get YouTube prompts easily
...
-- Step 8: Create a test function to verify YouTube setup
...
BEGIN
  ...
  RAISE NOTICE '2. Deploy the updated code to Vercel';
  RAISE NOTICE '3. Test with a YouTube URL in the Ideation page';
  RAISE NOTICE '';
